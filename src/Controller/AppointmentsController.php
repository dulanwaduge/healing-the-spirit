...
class AppointmentsController extends AppController
{
    ...
    {
        //Verfied Login $ IF login as Admin
        if (!empty($_SESSION["logininfo"]) && $_SESSION["logininfo"]["rule"] == "admin") {
            $email = isset($_SESSION["search_email"]) ? $_SESSION["search_email"] : "";
            $get = $this->request->getQueryParams();
            if ($email && (!empty($get["action"]) && $get["action"] == "cancel")) {
                $get["email"] = $email;
            }
            ...
            $this->set(compact('appointments', 'get'));
        } else {
            $this->redirect("/login");
        }
    }
...
//        $this->viewBuilder()->setLayout('client');
        ...
        if ($this->request->is('post')) {
            ...
            else $this->Flash->error(__('The appointment could not be saved. Please, try again.'));
        }
        $time = $this->time_arr;
