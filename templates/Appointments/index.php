    ...
    <div class="table-responsive">
        <form >
            <div id="dataTable_filter" class="dataTables_filter">
                <label>Search:<input type="text" value="<?=isset($get['email']) ? $get['email'] : ''?>" name="email" placeholder="email" aria-controls="dataTable" style="width: 30%; margin-right: 10px;">
                    <?= $this->Form->button(__('search'), ['class' => 'button']) ?>

                </label>
                <input type="hidden" name="action" value="<?=isset($get['action']) ? $get['action'] : ''?>">
            </div>
        </form>
            ...
            <?php foreach ($appointments as $appointment): ?>
                <tr>
                    ...
                    <td class="actions">
                        ...
                        <?= $this->Form->postLink(__('Cancel'), ['action' => 'cancel', $appointment->id], ['confirm' => __('Are you sure you want to cancle # {0}?', $appointment->id)]) ?>
